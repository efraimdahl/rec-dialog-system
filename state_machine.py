...
class RestaurantAgent(StateMachine):
    ...
    # INITIATION
    def __init__(self, restaurant_file: str, classifier_file: Union[ClassifierMixin, KeywordClassifier],
                 vectorizer_file: CountVectorizer) -> None:
            ...
            if (classAnswer[0] in ["inform", "reqalts", "confirm", "negate", "request"]):
                ...
                for key, val in classAnswer[1].items():
                    if key == "area":
                        ...
                        self.foodType = val
                    elif key == "priceRange" and (
                            RANDOMIZE_PREFERENCE_QUESTION_ORDER or (self.foodType != "" and self.area != "")):
                        self.priceRange = val
    ...
    def no_response_formatter(self, other: bool = False) -> str:
        ...
        pricepart = f'that has {self.priceRange} prices' if (
                    self.priceRange != "" and self.priceRange != "dontcare") else ""
    ...
    def on_enter_no_restaurant_found(self) -> None:
        ...
        if (self.no_res_passes > 0):
            chatbot_print(
                "Sorry, but there are no such restaurants, maybe try changing the location, area or foodtype?")
        else:
    ...
    def on_enter_return_restaurant(self) -> None:
        ...
        if (len(self.filteredRestaurants) < self.tries + 1):
            ...
            chatbot_print(
                f"{row['restaurantname']} is a nice place in the {row['area']} part of town serving {row['food']} food and the prices are {row['pricerange']}")
    ...
    def on_enter_give_information(self, input: str) -> None:
        ...
        if (self.current_suggestion_set):
            ...
            if response != "":
                ...
                return
            chatbot_print(
                "Can you provide specific information you are looking for such as phone number, area or address?")
